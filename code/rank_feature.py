...
def func_old_itemcf_last_sim(x):
    user_id = x['user_id']
    article_id = x['article_id']
...
if __name__ == '__main__':
    if mode == 'valid':
        df_feature = pd.read_pickle('/home/xiaoguzai/数据/新闻推荐/user_data/data/offline/try_recall.pkl')
        df_click = pd.read_pickle('/home/xiaoguzai/数据/新闻推荐/user_data/data/offline/click.pkl')

    else:
        df_feature = pd.read_pickle('/home/xiaoguzai/数据/新闻推荐/user_data/data/online/recall.pkl')
    ...
    #hot相关
    if mode == 'valid':
        f = open('/home/xiaoguzai/数据/新闻推荐/user_data/data/offline/recall_hot.pkl', 'rb')
        article_vec_map = pickle.load(f)
        f.close()

    # hot最热新闻的统计值
    article_vec_map = article_vec_map.rename(columns={'sim_score':'rank_score'})
    df_article = df_article.merge(article_vec_map, on=['article_id'], how='left')
    """
    ...
    log.debug(f'df_feature.head(): {df_feature.head()}')
    log.debug(f'df_feature.shape: {df_feature.shape}')
    log.debug(f'df_feature.columns: {df_feature.columns.tolist()}')

    print('new df_feature head = ')
    ...
    if mode == 'valid':
        ...
        f = open('/home/xiaoguzai/数据/新闻推荐/user_data/sim/online/itemcf_sim.pkl', 'rb')
        item_sim = pickle.load(f)
        f.close()
    ...
    if mode == 'valid':
        ...
        f = open('/home/xiaoguzai/数据/新闻推荐/user_data/sim/online/binetwork_sim.pkl', 'rb')
        binetwork_sim = pickle.load(f)
        f.close()
    ...
    ## w2v 相关
    if mode == 'valid':
        f = open('/home/xiaoguzai/数据/新闻推荐/user_data/data/offline/article_w2v.pkl', 'rb')
        article_vec_map = pickle.load(f)
        f.close()
    else:
        f = open('/home/xiaoguzai/数据/新闻推荐/user_data/data/online/article_w2v.pkl', 'rb')
        article_vec_map = pickle.load(f)
        f.close()
    ...
    r"""
    df_feature head = 
           user_id  article_id  label  sim_score  category_id  created_at_ts  \
    0        8      209122    0.0   2.758033          332     1508184837   
    1        8       50644    1.0   1.785602
    
    now article_vec_map = 
       user_id  article_id  sim_score  label
    0       34      272143   0.009980    0.0
    1       34      234698   0.009671    0.0
    2       34      123909   0.009570    0.0
    3       34      336221   0.009353    0.0
    4       34      336223   0.008678    0.0
    """

    # 保存特征文件
    if mode == 'valid':
        df_feature.to_pickle('/home/xiaoguzai/数据/新闻推荐/user_data/data/offline/try_feature.pkl')

    else:
        df_feature.to_pickle('/home/xiaoguzai/数据/新闻推荐/user_data/data/online/feature.pkl')
